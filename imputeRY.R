...
tables.RYsd <- lapply(files.RYse, function(x) read.csv(x, header = FALSE))
...
setwd("/Users/nicolekinlock/Documents/Plant Ecology/NetworkMetaAnalysis/Networks/RY/Impute/MixMono")
...
m <- jags(inits = InitStage,
          n.chains = nc,
          model.file = "imputeRYMixMono.jags",
...
ggplot(data = dat.output) + geom_point(aes(x = RY, y = RYsd)) + geom_line(aes(x = RY, y = exp(theta))) +
  ...
  coord_cartesian(xlim = c(0, 10), ylim = c(0, 100)) + ylab("RYsd") + xlab("RY") + ggtitle("Imputing RYsd") + theme_bw()
...
for (h in 1:length(tables.missing)) {
  ...
  write.table(x = tables.missing[[h]], file = paste("/Users/nicolekinlock/Documents/Plant Ecology/NetworkMetaAnalysis/Networks/RY/", substr(files.missing[h], 3, nchar(files.missing[h]) - 4), "-imp.csv", sep = ""), sep = ",", row.names = FALSE) 
}
