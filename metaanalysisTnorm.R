...
if (k == 2) {
  meta.analysis.init <- post.bootstrap.df[which(post.bootstrap.df$MetricName == "WeightedConnectance" | post.bootstrap.df$MetricName == "WeightedCompConnectance" | 
                                                  post.bootstrap.df$MetricName == "WeightedFacConnectance" | post.bootstrap.df$MetricName == "RelativeIntransitivity" |
...
for (a in 1:length(metric.names)) {
  ...
  MCMCtrace(meta.analysis.fit, excl = "deviance", pdf = TRUE, filename = paste("/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/Figures/Convergence/tnormal_overall_", metric.names[a], "_", rii.type.name, sep = ""), open_pdf = FALSE)
...
for (a in 1:length(metric.names)) {
  ...
  while (1 == 1) {
    meta.analysis.fit <- RunMetaAnalysis(num.observations = nrow(subset.df), observed.y = subset.df$ObservedMean, observed.var = subset.df$BootstrapVar, group.input = subset.df$Group,
                                         num.group = length(unique(subset.df$Group)), parameter.inits = GenerateInitsGroupwise, save.parameters = c("beta", "overall.sd"), file.jags = ma.tnormal.group.file)
    if (all(meta.analysis.fit$BUGSoutput$summary[, 8] < gr.threshold & meta.analysis.fit$BUGSoutput$summary[, 9] > neff.threshold)) {
      break
    }
  }
  MCMCtrace(meta.analysis.fit, excl = "deviance", pdf = TRUE, filename = paste("/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/Figures/Convergence/tnormal_habitat_", metric.names[a], "_", rii.type.name, sep = ""), open_pdf = FALSE)
  output <- ExtractJAGSOutput(meta.analysis.fit, groupwise = TRUE)
  output.add <- data.frame(ModelType = rep("Habitat", nrow(output)), NumCases = c(sum(subset.df$Group == 1), sum(subset.df$Group == 2), nrow(subset.df)), Distribution = rep("TNormal", nrow(output)), output)
...
for (a in 1:length(metric.names)) {
  ...
  while (1 == 1) {
    meta.analysis.fit <- RunMetaAnalysis(num.observations = nrow(subset.df), observed.y = subset.df$ObservedMean, observed.var = subset.df$BootstrapVar, group.input = subset.df$Group,
                                         ...
                                         file.jags = ma.tnormal.group.file)
    if (all(meta.analysis.fit$BUGSoutput$summary[, 8] < gr.threshold & meta.analysis.fit$BUGSoutput$summary[, 9] > neff.threshold)) {
      break
    }
  }
  MCMCtrace(meta.analysis.fit, excl = "deviance", pdf = TRUE, filename = paste("/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/Figures/Convergence/tnormal_setting_", metric.names[a], "_", rii.type.name, sep = ""), open_pdf = FALSE)
  output <- ExtractJAGSOutput(meta.analysis.fit, groupwise = TRUE)
  output.add <- data.frame(ModelType = rep("Setting", nrow(output)), NumCases = c(sum(subset.df$Group == 1), sum(subset.df$Group == 2), sum(subset.df$Group == 3), nrow(subset.df)),  Distribution = rep("TNormal", nrow(output)), output)
  meta.analysis.df <- rbind(meta.analysis.df, output.add)
}
...
for (a in 1:length(metric.names)) {
  ...
  MCMCtrace(meta.analysis.fit, excl = "deviance", pdf = TRUE, filename = paste("/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/Figures/Convergence/tnormal_herbwood_", metric.names[a], "_", rii.type.name, sep = ""), open_pdf = FALSE)
  output <- ExtractJAGSOutput(meta.analysis.fit, groupwise = TRUE)
  output.add <- data.frame(ModelType = rep("Habit", nrow(output)),NumCases = c(sum(subset.df$Group == 1), sum(subset.df$Group == 2), nrow(subset.df)), Distribution = rep("TNormal", nrow(output)), output)
  meta.analysis.df <- rbind(meta.analysis.df, output.add)
}
...
for (a in 1:length(metric.names)) {
  ...
  MCMCtrace(meta.analysis.fit, excl = "deviance", pdf = TRUE, filename = paste("/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/Figures/Convergence/tnormal_age_", metric.names[a], "_", rii.type.name, sep = ""), open_pdf = FALSE)
  output <- ExtractJAGSOutput(meta.analysis.fit, groupwise = TRUE)
  output.add <- data.frame(ModelType = rep("Age", nrow(output)), NumCases = c(sum(subset.df$Group == 1), sum(subset.df$Group == 2), nrow(subset.df)), Distribution = rep("TNormal", nrow(output)), output)
  meta.analysis.df <- rbind(meta.analysis.df, output.add)
}
...
write.csv(x = meta.analysis.by.network, file = paste("/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/MetaAnalysis_ByNetwork_TNormal_", rii.type.name, ".csv", sep = ""))
write.csv(x = meta.analysis.df, file = paste("/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/MetaAnalysis_TNormal_", rii.type.name, ".csv", sep = ""))
