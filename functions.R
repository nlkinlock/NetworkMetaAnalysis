...
SpecialCaseRII.Costa <- function(df, num.species, type) {
  if (type == "bootstrap") {
    rii.matrix <- c()
    rii.matrix[1] <- (mean(df[, 11]) - mean(df[, 8])) / (mean(df[, 11]) + mean(df[, 8]))
    rii.matrix[2] <- (mean(df[, 5]) - mean(df[, 1])) / (mean(df[, 5]) + mean(df[, 1]))
    rii.matrix[3] <- (mean(df[, 17]) - mean(df[, 14])) / (mean(df[, 17]) + mean(df[, 14]))
    rii.matrix[4] <- (mean(df[, 10]) - mean(df[, 7])) / (mean(df[, 10]) + mean(df[, 7]))
    rii.matrix[5] <- (mean(df[, 4]) - mean(df[, 2])) / (mean(df[, 4]) + mean(df[, 2]))
    rii.matrix[6] <- (mean(df[, 16]) - mean(df[, 13])) / (mean(df[, 16]) + mean(df[, 13]))
    rii.matrix[7] <- (mean(df[, 12]) - mean(df[, 9])) / (mean(df[, 12]) + mean(df[, 9]))
    rii.matrix[8] <- (mean(df[, 6]) - mean(df[, 3])) / (mean(df[, 6]) + mean(df[, 3]))
    rii.matrix[9] <- (mean(df[, 18]) - mean(df[, 15])) / (mean(df[, 18]) + mean(df[, 15]))
  } else if (type == "observed") {
    one.one <- (df$Metric[11] - df$Metric[8]) / (df$Metric[11] + df$Metric[8])
    two.one <- (df$Metric[10] - df$Metric[7]) / (df$Metric[10] + df$Metric[7])
    three.one <- (df$Metric[12] - df$Metric[9]) / (df$Metric[12] + df$Metric[9])
    one.two <- (df$Metric[5] - df$Metric[1]) / (df$Metric[5] + df$Metric[1])
    two.two <- (df$Metric[4] - df$Metric[2]) / (df$Metric[4] + df$Metric[2])
    three.two <- (df$Metric[6] - df$Metric[3]) / (df$Metric[6] + df$Metric[3])
    one.three <- (df$Metric[17] - df$Metric[14]) / (df$Metric[17] + df$Metric[14])
    two.three <- (df$Metric[16] - df$Metric[13]) / (df$Metric[16] + df$Metric[13])
    three.three <- (df$Metric[18] - df$Metric[15]) / (df$Metric[18] + df$Metric[15])
...
RunMetaAnalysis <- function(num.observations, observed.y, observed.var, num.group, group.input, sample.size, observed.counts, parameter.inits, save.parameters, file.jags) {
  if(hasArg(num.group) & !hasArg(sample.size)) {
    jags.data <- list(num.observations = num.observations, observed.y = observed.y, observed.var = observed.var, num.group = num.group, group = group.input)
    ...
    jags.data <- list(num.observations = num.observations, sample.size = sample.size, observed.counts = observed.counts, num.group = num.group, group = group.input)
  } else {
  ...
  jags.fit <- jags(inits = parameter.inits, n.chains = num.chains, model.file = file.jags, working.directory = getwd(),
                   data = jags.data, parameters.to.save = save.parameters, n.thin = thinning.rate, n.iter = chain.length, n.burnin = num.burnin, DIC = TRUE)
  return(jags.fit)
}
...
PosteriorPredictiveCheck <- function(fit, fit.func, obs.dat, dist.name) {
  if(any(fit$parameters.to.save == "param2")) {
    ...
    posterior.predictive.median <- melt(apply(posterior.predictive, 2, median), value.name = "Median")
    ...
    posterior.predictive.mean <- melt(apply(posterior.predictive, 2, mean), value.name = "Mean")
    posterior.predictive.mean.proportion <- length(which(posterior.predictive.mean >= mean(obs.dat))) / nrow(fit$BUGSoutput$sims.matrix)
    posterior.predictive.upper <- melt(apply(posterior.predictive, 2, function(x) quantile(x, 0.9)), value.name = "Upper90")
    ...
    posterior.predictive.out <- data.frame(metric = metric.names[m], Distribution = dist.name, param1 = fit$BUGSoutput$mean$param1, param2 = fit$BUGSoutput$mean$param2,
                                           D = fit$BUGSoutput$mean$deviance, PPPmedian = posterior.predictive.median.proportion, PPPmean = posterior.predictive.mean.proportion,
...
SamplePosteriorNetwork <- function(models.init, dist.name, dist.func) {
  ...
  samp <- melt(samp)
  df <- data.frame(Case = samp[, 2], Type = rep("Fitted", nrow(samp)), Distribution = rep(dist.name, nrow(samp)), Fit = rep(models$Fit, each = sample.size), Values = samp$value)
  return(df)
}
...
SamplePosteriorParetoNetwork <- function(models) {
  ...
  samp <- melt(sim)
  df <- data.frame(Case = samp[, 2], Type = rep("Fitted", nrow(samp)), Distribution = rep("Pareto", nrow(samp)), Fit = rep(models[which(models$Distribution == "Pareto"), "Fit"], each = sample.size), Values = samp$value)
}
...
EstimateMeanDifferences <- function(s.in.obs, s.out.obs, sigma.in.obs, sigma.out.obs, groupID.in, groupID.out) {
  jags.data <- list(s.in.obs = s.in.obs, s.out.obs = s.out.obs, sigma.in.obs = sigma.in.obs, sigma.out.obs = sigma.out.obs, groupID.in = groupID.in, groupID.out = groupID.out)
  jags.inits <- function() {
...
ExtractAdditiveJAGSModel <- function(fit) {
  ...
  output <- data.frame(Parameter = c("grand mean", rep("group mean", 4), "among-group sd", "within-group sd"), 
                       Network = c("All", name, "All", "All"), Mean = output.raw[, 1], CIL = output.raw[, 2], CIU = output.raw[, 3], GR = output.raw[, 4], EffectiveN = output.raw[, 5])
  return(output)
}
