...
if (any(colnames(dat) == "Yield")) {
  boot.ry <- matrix(NA, nrow = R, ncol = sp^2)
  for (r in 1:R) {
    temp <- rtruncnorm(n = nrow(dat), mean = dat$Yield, sd = dat$Yield_SD, a = 0)
    for (i in 1:sp) {
      mono <- which(dat$Target == i & dat$Neighbor == i)
      for (j in 1:sp) {
        mix <- which(dat$Target == i & dat$Neighbor == j)
        if (i == j) {
          ...
          boot.ry[r, mix] <- (temp[mix] / temp[mono])
        }
      }
    }
  }
    ...
    for (i in 1:sp) {
      ...
      for (j in 1:sp) {
        ...
        if (i == j) {
          ...
          boot.ry[r, mix] <- dat$p[mix] * (temp[mix] / temp[mono])
        }
      }
    }
  }
