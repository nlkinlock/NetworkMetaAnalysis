...
#
tab.init <- read.csv(paste(path, "Input/BiomassData/1787_Weigelt_2002.csv", sep = ""))
...
for (s in 1:length(name)) {
  ...
  write.table(x = dat, file = paste(path, "Input/CaseData/", name[s], ".csv", sep = ""), sep = ",", row.names = FALSE)
}
