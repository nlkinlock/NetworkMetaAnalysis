...
#
biomass <- read.csv("/home/nlkinlock/Documents/PlantInteractionNetworks/Input/FinalSeedlingMeasurements.csv")
...
dat.ss <- ddply(ss, .(UniqueID), summarise, SD = sd(AbovegroundBiomass, na.rm = TRUE), 
                Metric = mean(AbovegroundBiomass, na.rm = TRUE), N = length(UniqueID))
...
dat.as <- ddply(as, .(UniqueID), summarise, SD = sd(AbovegroundBiomass, na.rm = TRUE), 
                Metric = mean(AbovegroundBiomass, na.rm = TRUE), N = length(UniqueID))
...
# save data tables ------------------------------------------------
#
write.table(x = dat.as, file = paste(path, "Input/CaseData/9998_Kinlock_unpubl.csv", sep = ""), sep = ",", row.names = FALSE)
write.table(x = dat.ss, file = paste(path, "Input/CaseData/9999_Kinlock_unpubl.csv", sep = ""), sep = ",", row.names = FALSE)
