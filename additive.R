...
additive <- data.frame(Network = character(), Target = character(), Neighbor.j = character(), Neighbor.k = character(), 
                        Difference = character(), stringsAsFactors = FALSE)
...
# number of spp
  ...
  for (i in 1:species) {
    ...
    for (j in 1:species) {
      ...
      for (k in 1:species) {
        # filter loop to only find values for which a 3-spp mixture was measured
        if (k == i | k == j | k < j | length(which(multispp[[x]]$Target == i & multispp[[x]]$Neighbor == paste(j, k, sep = ""))) == 0) {
          next
        }
...
# store raw output
write.csv(additive, file = paste(path, "Output/AdditivityRawOutput.csv", sep = ""))
#
...
MCMCtrace(add.fit, open_pdf = FALSE, excl = "deviance", pdf = TRUE, filename = "additivity", 
            wd = paste(path, "Output/Figures/Convergence", sep = ""))
add.output <- ExtractAdditiveJAGSModel(add.fit)
# store output from model
write.csv(add.output, file = paste(path, "Output/AdditivityFit.csv"))
