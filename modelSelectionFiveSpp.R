...
for (i in 1:2) {
  model.selection <- data.frame(Case = character(), metric = character(), Distribution = character(), param1 = numeric(), param2 = numeric(), 
                                ...
                                pWAIC = numeric(), WAIC = numeric(), SEelpdWAIC = numeric(), SEpWAIC = numeric(), SEWAIC = numeric(), stringsAsFactors = FALSE)
  ...
  if (i == 1) {
    ...
    setwd("/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/Networks/TrueControl/")
  }
  ...
  for (n in 1:length(networks.all)) {
    ...
    metric.list <- list(abs(in.strength[!is.na(in.strength) & in.strength != 0]), abs(out.strength[!is.na(out.strength) & out.strength != 0]))  # if strength is zero, this means entire row/col was NA
    ...
    for (m in 1:length(metric.names)) {
      ...
      #
      MCMCtrace(object = normal.fit, params = c("param1", "param2"),  pdf = TRUE, filename = paste(case.names[n], "Normal", metric.names[m], rii.type.file.name, sep = "_"), wd = "/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/Figures/Convergence/ModelSelection", open_pdf = FALSE)
      MCMCtrace(object = lnormal.fit, params = c("param1", "param2"),  pdf = TRUE, filename = paste(case.names[n], "Lognormal", metric.names[m], rii.type.file.name, sep = "_"), wd = "/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/Figures/Convergence/ModelSelection", open_pdf = FALSE)
      MCMCtrace(object = exponential.fit, params = c("param1"),  pdf = TRUE, filename = paste(case.names[n], "Exponential", metric.names[m], rii.type.file.name, sep = "_"), wd = "/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/Figures/Convergence/ModelSelection", open_pdf = FALSE)
      MCMCtrace(object = pareto.fit, params = c("param1", "param2"),  pdf = TRUE, filename = paste(case.names[n], "Pareto", metric.names[m], rii.type.file.name, sep = "_"), wd = "/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/Figures/Convergence/ModelSelection", open_pdf = FALSE)
      #
  ...
  #
  colnames(model.selection) <- c("Case", "Metric", "Distribution", "param1", "param2", "D", "PPPmedian", "PPPmean", "PPPupper", "elpdWAIC", "pWAIC", "WAIC", "SEelpdWAIC", "SEpWAIC", "SEWAIC")
  ...
  write.csv(x = model.selection, file = paste("/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/ModelSelectionSubset_", rii.type.file.name, ".csv", sep = ""), row.names = FALSE)
  ...
  mono.ctrl.levels <- c("Armas & Pugnaire 2011", "Svenning et al. 2008", "Niu & Wan 2008", "Mariotte et al. 2012", 
    "Jiang et al. 2014", "Miller & Werner 1987", "Engel & Weltzin 2008", "Goldberg & Landa 1991", "Kinlock unpublished (b)", "Kinlock unpublished")
  mono.ctrl.levels.ordered <- c("Miller & Werner 1987", "Engel & Weltzin 2008", "Niu & Wan 2008", "Mariotte et al. 2012", "Jiang et al. 2014", 
    "Goldberg & Landa 1991", "Svenning et al. 2008", "Armas & Pugnaire 2011", "Kinlock unpublished (b)", "Kinlock unpublished")
  true.ctrl.levels <- c("Armas & Pugnaire 2011", "Svenning et al. 2008", "Löf et al. 2014", "Goldberg & Landa 1991", "Kinlock unpublished (b)",
                        "Kinlock unpublished")
  true.ctrl.levels.ordered <- c("Goldberg & Landa 1991", "Svenning et al. 2008", "Armas & Pugnaire 2011", "Löf et al. 2014", "Kinlock unpublished (b)", "Kinlock unpublished")
  
  # metric 1: in-strength
  models.in.strength <- model.selection[which(model.selection$Metric == "In-strength"), ]
  
  in.strength.init <- lapply(networks.all, function(x) colSums(t(x), na.rm = TRUE))
  names.row <- c()
  ...
  write.csv(dist.in.strength, paste("/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/InStrengthSamples_", rii.type.file.name, ".csv", sep = ""))
  ...
  out.strength.init <- lapply(networks.all, function(x) colSums(t(x), na.rm = TRUE))
  ...
  write.csv(dist.out.strength, paste("/Users/nicolekinlock/Documents/NetworkMetaAnalysis/Output/OutStrengthSamples_", rii.type.file.name, ".csv", sep = ""))
}
