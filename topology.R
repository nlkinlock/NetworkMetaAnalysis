...
metrics <- list(abs(weight), abs(s.in), abs(s.in.c), abs(s.in.f), abs(s.out), abs(s.out.c), abs(s.out.f))
...
# rownames(param2) <- c("weight", "s.in", "s.in.c", "s.in.f", "s.out", "s.out.c", "s.out.f")
...
for (i in 1:length(metrics)) {
  ...
  # power law
  # pl <- conpl(metrics[[i]])
  # try(est <- estimate_xmin(pl, xmax = Inf))
  # try(pl$setXmin(est))
  # try(est <- estimate_pars(pl))
  # try(fit.pl <- dist_ll(pl))
  # try(result <- c(pars = pl$pars, n = length(pl$dat), LL = fit.pl))
  # if (any(is.na(result))) {
  #   aicc[5] <- NA
  #   theta1[5] <- NA
  # } else {
  # aicc[5] <- -2 * result[3] + 2 * 1 + ((2 * 1 * (1 + 1)) / result[2] - 1 - 1)
  # theta1[5] <- result[1]
  # }
  # theta2[5] <- NA
...
all.aicc <- data.frame(all.aicc, rep(count, nrow(all.aicc)))
colnames(all.aicc) <- c("Metric", "Distribution", "AICc", "Iteration")
